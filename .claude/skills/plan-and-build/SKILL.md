...
~/.claude/skills/planning/  (글로벌 공용 코어, symlink → ~/personal/fos-skills/planning)
...
## 레이어 phase 가이드 (backend-fos)

도메인 기반 패키지 구조: `presentation/ → application/ → domain/ → infra/` (각 도메인 내부 단방향 의존성).

새 기능 추가 시 권장 phase 분리:

| Phase | 내용 | 모델 |
|---|---|---|
| 1 | Domain (Entity, Value Object, Repository 인터페이스) | sonnet |
| 2 | Infra (Repository 구현체, Flyway 마이그레이션 `V{timestamp}__*.sql`) | sonnet |
| 3 | Application (Service, `@Transactional` 경계, AOP, Event Listener) | sonnet |
| 4 | Presentation (Controller, Request/Response DTO, SpringDoc) | sonnet |
| 5 | 통합 테스트 (`AbstractControllerTest`/`TestFixturesSupport` 상속) | sonnet |
| N-1 | 빌드 + Checkstyle 검증 (`./gradlew checkstyleMain checkstyleTest test build -x integrationTest --no-daemon`) | haiku |
| N | 커밋 + push | haiku |

**Phase 분리 원칙**: 각 레이어는 자기완결적. Domain 변경 없이 Infra만 건드리는 phase, Service 변경 없이 Controller만 건드리는 phase는 서로 분리.

**Flyway 마이그레이션 주의**: 기존 적용 파일 수정 절대 금지 (checksum 충돌). 새 파일은 타임스탬프 기반 버전이 기존보다 항상 뒤에 오도록.
