...
public class AuthController {
    ...
    @Operation(
            summary = "회원가입",
            description = "새로운 사용자를 등록하고 JWT 토큰을 발급합니다. (NextAuth OAuth 연동용)",
            security = {}  // 이 엔드포인트는 인증 불필요
    )
    ...
    public ResponseEntity<ApiSuccessResponse<AuthResponse>> register(
        ...
        log.info("User registration/login request: {}", request.getEmail());
    ...
    @Operation(
            summary = "로그인",
            description = "이메일로 사용자를 조회하고 JWT 토큰을 발급합니다.",
            security = {}  // 이 엔드포인트는 인증 불필요
    )
    ...
    @Operation(
            summary = "로그아웃",
            description = "로그아웃 처리. JWT는 stateless이므로 클라이언트에서 토큰을 삭제하면 됩니다.",
            security = @SecurityRequirement(name = "bearerAuth")
    )
