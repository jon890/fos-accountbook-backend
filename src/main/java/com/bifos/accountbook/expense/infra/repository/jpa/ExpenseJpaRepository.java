package com.bifos.accountbook.expense.infra.repository.jpa;
...
public interface ExpenseJpaRepository extends JpaRepository<Expense, Long> {
  ...
  @Query("SELECT COALESCE(SUM(e.amount), 0) " +
      "FROM Expense e " +
      "LEFT JOIN Category c ON e.categoryUuid = c.uuid AND c.status = com.bifos.accountbook.category.domain.value.CategoryStatus.ACTIVE " +
      "WHERE e.family.uuid = :familyUuid " +
      "AND e.status = com.bifos.accountbook.expense.domain.value.ExpenseStatus.ACTIVE " +
      "AND e.date BETWEEN :startDate AND :endDate " +
