package com.bifos.accountbook.income.infra.repository.jpa;
...
public interface IncomeJpaRepository extends JpaRepository<Income, Long> {
  ...
  @Query("SELECT i FROM Income i WHERE i.uuid = :uuid AND i.status = com.bifos.accountbook.income.domain.value.IncomeStatus.ACTIVE")
  Optional<Income> findActiveByUuid(@Param("uuid") CustomUuid uuid);

  @Query("SELECT i FROM Income i WHERE i.family.uuid = :familyUuid AND i.status = com.bifos.accountbook.income.domain.value.IncomeStatus.ACTIVE ORDER BY i.date DESC")
  Page<Income> findAllByFamilyUuid(@Param("familyUuid") CustomUuid familyUuid, Pageable pageable);

  @Query("SELECT i FROM Income i WHERE i.family.uuid = :familyUuid AND i.date BETWEEN :startDate AND :endDate AND i.status = com.bifos.accountbook.income.domain.value.IncomeStatus.ACTIVE ORDER BY i.date DESC")
  List<Income> findByFamilyUuidAndDateBetween(
      @Param("familyUuid") CustomUuid familyUuid,
      @Param("startDate") LocalDateTime startDate,
      @Param("endDate") LocalDateTime endDate);

  @Query("SELECT i FROM Income i WHERE i.family.uuid = :familyUuid AND i.categoryUuid = :categoryUuid AND i.status = com.bifos.accountbook.income.domain.value.IncomeStatus.ACTIVE ORDER BY i.date DESC")
  ...
  @Query("SELECT i FROM Income i WHERE i.family.uuid = :familyUuid " +
      ...
      "AND i.status = com.bifos.accountbook.income.domain.value.IncomeStatus.ACTIVE " +
