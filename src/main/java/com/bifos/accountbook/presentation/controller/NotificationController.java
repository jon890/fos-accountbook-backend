...
public class NotificationController {
    ...
    public ResponseEntity<ApiSuccessResponse<NotificationListResponse>> getFamilyNotifications(
        ...
        log.info("Fetching notifications for family: {} by user: {}", familyUuid, loginUser.getUserUuid());

        NotificationListResponse response = notificationService.getFamilyNotifications(
                loginUser.getUserUuid(), familyUuid);
    ...
    public ResponseEntity<ApiSuccessResponse<Map<String, Long>>> getUnreadCount(
            @LoginUser LoginUserDto loginUser,
            @Parameter(description = "가족 UUID") @PathVariable String familyUuid) {
        Long unreadCount = notificationService.getUnreadCount(loginUser.getUserUuid(), familyUuid);
    ...
    public ResponseEntity<ApiSuccessResponse<NotificationResponse>> getNotification(
        ...
        log.info("Fetching notification: {} by user: {}", notificationUuid, loginUser.getUserUuid());

        NotificationResponse response = notificationService.getNotification(
                loginUser.getUserUuid(), notificationUuid);
    ...
    public ResponseEntity<ApiSuccessResponse<NotificationResponse>> markAsRead(
            @LoginUser LoginUserDto loginUser,
            @Parameter(description = "알림 UUID") @PathVariable String notificationUuid) {
        log.info("Marking notification as read: {} by user: {}", notificationUuid, loginUser.getUserUuid());
    ...
    public ResponseEntity<ApiSuccessResponse<Void>> markAllAsRead(
            @LoginUser LoginUserDto loginUser,
            @Parameter(description = "가족 UUID") @PathVariable String familyUuid) {
        log.info("Marking all notifications as read for family: {} by user: {}", familyUuid, loginUser.getUserUuid());

        notificationService.markAllAsRead(loginUser.getUserUuid(), familyUuid);
